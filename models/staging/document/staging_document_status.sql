...
UNION ALL -- new logic with log_type and edition_type since July 2026
...
(
    with document_events as (
        ...
        from {{ ref('staging_tenant_log') }}
        where
            log_type in ('DOCUMENT_ADDED','DOCUMENT_DELETED')
    )
    ...
    , document_status as (
        select distinct
            ...
            , SUM(case when log_type = 'DOCUMENT_ADDED' then 1 else 0 end) over (
                partition by document_id
                rows between unbounded preceding and unbounded following
            ) as total_add_document_events
            , SUM(case when log_type = 'DOCUMENT_DELETED' then 1 else 0 end) over (
                partition by document_id
                rows between unbounded preceding and unbounded following
            ) as total_delete_document_events
        from document_events
    ...
    select
        ...
        , document_sub_category
        , case
            when last_document_events = 'DOCUMENT_ADDED' then 'ADDED'
            when last_document_events = 'DOCUMENT_DELETED' then 'DELETED'
        end as document_status
        , first_created_at as added_at
        , case
            when last_document_events = 'DOCUMENT_DELETED' then last_created_at
        end as deleted_at
        , total_add_document_events
        , total_delete_document_events
    from document_status
