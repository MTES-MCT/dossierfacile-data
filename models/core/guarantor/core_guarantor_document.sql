...
, guarantor_document_status as (
    select distinct
        guarantor_id
        , tenant_id
        , COUNT(identification_sub_category) over (partition by guarantor_id) as nb_identification_documents
        , LAST_VALUE(identification_sub_category) over (
            partition by guarantor_id
        ...
        , COUNT(financial_sub_category) over (partition by guarantor_id) as nb_financial_documents
        ...
        , COUNT(residency_sub_category) over (partition by guarantor_id) as nb_residency_documents
        ...
        , COUNT(professional_sub_category) over (partition by guarantor_id) as nb_professional_documents
        ...
        , COUNT(tax_sub_category) over (partition by guarantor_id) as nb_tax_documents
        ...
        , SUM(monthly_net_income) over (partition by guarantor_id) as monthly_net_income
    from pivot_guarantor_documents
)
